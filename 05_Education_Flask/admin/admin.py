...
# We can't use 'flsk-login' because it already using in app.py
def login_admin():
...
def logout_admin():
    session.pop('admin_logged', None) # method 'pop' delete 'admin_logged' from session
...
db = None
@admin.before_request # For connection with database
def before_request():
    global db
    # 'g' global variable of aplication's context. 'link_db' - g's property
    db = g.get('link_db') # Better make: current_app.config['DATABASE']
...
def login():
    ...
    if request.method == 'POST':
        if request.form['user'] == 'admin' and request.form['psw'] == '12345':
            login_admin()
            # '.' means that 'index' must take from the current blueprint (or may 'admin.index')
            return redirect(url_for('.index'))
        else:
